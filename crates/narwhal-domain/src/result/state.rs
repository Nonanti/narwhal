...
//!
//! Moved out of `narwhal-app::core::state::result` so any host that
//! wants to reason about query outcomes (tests, embedders, MCP) can
//! name the same shapes without taking the full app crate along.
//!
//! `DiagramModalState` and `DiagramMode` stay in the app layer — they
//! carry `narwhal-diagram` field types, and `narwhal-diagram` already
//! depends on `narwhal-domain`. See
//! `narwhal-app::core::state::diagram_modal`.
...
use crate::export::QualifiedName;
use crate::result::{ExplainPlanLine, MetaTab, ResultView};
