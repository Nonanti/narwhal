...
impl StatusBar {
    ...
    /// wall-clock tick. So with no other activity the notification
    /// can linger on screen past `ttl` until the next triggering
    /// event. For the only current caller (multi-line paste warning,
    /// `Duration::from_secs(3)`) this is acceptable: the very next
    /// keystroke clears it. Callers that need a hard deadline should
    /// also wake the draw scheduler at `expires_at`.
