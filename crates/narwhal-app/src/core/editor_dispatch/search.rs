...
impl AppCore {
    pub(crate) fn open_editor_search(&mut self, direction: SearchDirection) {
        ...
        self.ui.status.message = format!("{prompt_char}");
    }
        ...
        match key.code {
            ...
            CtKey::Enter => {
                ...
                let needle = self.ui.tabs[self.ui.active_tab]
                    ...
                    .clone();
                if count == 0 {
                    self.ui.status.message = format!("/{needle} · no matches");
                } else {
                    let idx = self.ui.tabs[self.ui.active_tab]
                        .editor_search
                        .current
                        .map_or(1, |i| i + 1);
                    self.ui.status.message = format!("/{needle} · {idx}/{count}");
                }
            }
            CtKey::Backspace => {
                ...
                let needle = self.ui.tabs[self.ui.active_tab]
                    ...
                    .needle
                    .clone();
                let prompt_char = match self.ui.tabs[self.ui.active_tab].editor_search.direction {
                    SearchDirection::Forward => '/',
                    SearchDirection::Backward => '?',
                    // Future search directions: default to forward prompt.
                    _ => '/',
    ...
    pub(crate) fn jump_to_editor_search_match(&mut self) {
        ...
        if let Some(i) = idx {
            let (row, col) = self.ui.tabs[self.ui.active_tab].editor_search.matches[i];
            self.ui.tabs[self.ui.active_tab].editor.set_cursor(row, col);
            self.ui.tabs[self.ui.active_tab].editor_search.current = Some(i);
        }
    }
    ...
    pub(crate) fn sync_editor_search_current(&mut self) {
        ...
        self.ui.tabs[self.ui.active_tab].editor_search.current = current;
    }
    ...
    pub(crate) fn repeat_editor_search(&mut self, reverse: bool) {
        ...
        if tab.editor_search.matches.is_empty() {
            self.ui.status.message = format!("/{} · no matches", tab.editor_search.needle);
            return;
        }
        let direction = tab.editor_search.direction;
        ...
        let (row, col) = self.ui.tabs[self.ui.active_tab].editor_search.matches[next];
        self.ui.tabs[self.ui.active_tab].editor.set_cursor(row, col);
        self.ui.tabs[self.ui.active_tab].editor_search.current = Some(next);
        let needle = self.ui.tabs[self.ui.active_tab]
            .editor_search
            .needle
            .clone();
        self.ui.status.message = format!("/{needle} · {}/{count}", next + 1);
    }
    ...
    pub(crate) fn execute_substitute(
        ...
        if confirm {
            ...
            self.ui.status.message = "confirm flag not yet supported; replacing all matches".into();
        }

        let total_replacements = match range {
            crate::commands::SubstituteRange::CurrentLine => {
                let row = self.ui.tabs[self.ui.active_tab].editor.cursor_row();
                let line = self.ui.tabs[self.ui.active_tab]
                    .editor
                    .get_line(row)
                    .to_owned();
            ...
            crate::commands::SubstituteRange::WholeBuffer => {
                let line_count = self.ui.tabs[self.ui.active_tab].editor.line_count();
        ...
        if total_replacements == 0 {
            self.ui.status.message = format!("{pattern} not found");
        } else {
            self.ui.status.message = format!("{total_replacements} replacement(s) made");
        }
    }
}
