...
Design notes for the SQL parser integration introduced in v2.0. The
LSP and multi-cursor subsystems build on top of the `Scope` /
`ScopeKind` contract documented here.
...
3. **`Scope` / `ScopeKind`** — given a byte offset, classifies the
  ...
  `JoinCondition`, …). This is the contract (LSP) and (multi-cursor) read.
...
## Scope contract for Tier 2 (LSP client) and (multi-cursor) consume the scope
