...
Design notes for the wasmtime-backed plugin runtime introduced in
v2.0. The MCP-tool plugin track and the capability sandbox
(documented in [`wasm-sandbox.md`](./wasm-sandbox.md)) build on the
...
that the MCP-tool track will edit. Doing it now
would force two merges of the same module — the discipline matches
the / pattern (defer cross-cutting wiring to a
...
## contract is the task that depends most directly on the output. The contract it builds against:

1. **Runtime construction**: reuses `narwhal_plugin_wasm::Runtime`
  exactly as the v0.1 lifecycle plugins do. A second WIT world
  (`mcp-tool` — to be defined) lives alongside the v0.1
...
## Capability boundary intentionally implements only the **policy match**: the
...
etc. — is **'s** scope. The boundary between the two tasks:
...
| Audit log of capability denials  |  —  |  ✅  | returns capability denials as a polite `ok=false` result
record so plugins can detect the missing wiring; will flip
...
| Both Lua and WASM plugins receive event stream  |  ✅ (`PluginRegistry::broadcast_event`; unit test covers both runtimes registered together) |
...
| Definition of Done passes  |  ✅ (fmt, clippy -D warnings, rustdoc -D warnings, all tests dev+release; 1041 tests vs's 984 — 57 net additions) |
...
before v2.0 GA. ships the *runtime* and the *contract*; the
follow-up plugs the runtime into `AppCore` and lights up the
end-to-end test.

## Convention notes for the next / agent
...
- The WIT file (`wit/world.wit`) is **append-only** across v2.x.
  ...
  semantics; renames or removals require an `api-version` bump and a entry.
- Host functions live in `src/host.rs` and follow the wasmtime 45
  `bindgen!` convention: import functions return
  `wasmtime::Result<T>` so capability denials can later become traps without breaking the trait shape.
