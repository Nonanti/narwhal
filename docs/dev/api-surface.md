# API surface audit — narwhal 2.0

Generated for the v1 → v2 migration guide. Re-run manually whenever
a new public item lands.

## Headline numbers

| Metric  | v1.2.0 | v2.0 (done) | Delta |
| ------------------------------------- | -----: | -----------------: | ----- |
...
When the toolchain regains access to cargo-public-api (e.g. in CI), will produce a machine-generated diff. The list below is the
human-readable baseline.

## Workspace crates after | Crate  | Role  | Public surface size |
| ---------------------- | --------------------------------------------- | ------------------: |
...
backend. The umbrella also gains `all-drivers` so the
...
> links to the relevant task in the roadmap.
...
pub use connection::{Connection, ConnectionColor, ConnectionConfig,
  ConnectionParams, DynConnection,  //
...
- added the `migrate` module and 12 new struct/enum names
...
#[cfg(feature = "mssql")]  pub mod mssql;  //
pub mod registry;
...
No changes vs v1.2.0 except via the workspace-wide effects (Rust 2024 use-group ordering) and (Dyn* trait renames
...
(verified via `git log -G '#\\[deprecated'`), so's "remove
...
- **Unused-deps audit**: `cargo build --workspace --all-targets` is
  ...
  of `unused_*` lints. `async-trait` removal from `narwhal-core` was confirmed by re-running the build with the dep gone.
- **Cargo feature audit**: `narwhal-drivers/Cargo.toml` features
  ...
  binary). Verified during. should re-run cargo-public-api in a richer environment and
attach the diff. The numbers above are the v2-snapshot.
...
- Set up `cargo-public-api` in CI (a follow-up). Producing
