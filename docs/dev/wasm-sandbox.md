# WASM plugin sandbox (capability + resource limits)

Design notes for the capability-based sandbox layered on top of the
plugin runtime (v2.0). Pairs with the runtime walkthrough at
[`wasm-runtime.md`][a]. The MCP-tool plugin track consumes the
capability model documented here.
...
`narwhal::plugin::audit`. landed the *policy match* (manifest declares → host policy
validates → load refused). raises that to per-call
...
* `Capability::FsRead` etc. went from unit variants to
  ...
  the widest scope so on-disk manifests keep loading.
...
settings parsing in the follow-up. Decoupling the
...
path is one map lookup. Microbenchmarking is deferred
...
The KV budget now **traps** on overrun. The trap upgrade matches the boundary table promise
...
`WasmPlugin::deliver_event` / `WasmPlugin::dispatch` exactly as did; the meter type centralises the math.
...
the `WasmPluginSettings` shape.

## contract reuses [`Capability`] + [`Grants`] + [`Enforcer`] verbatim.
...
identically to lifecycle plugins. does **not** need to fork
...
* **App-side wiring.** Same boundary as: a single focused
  follow-up plugs the runtime into `AppCore`. Avoids two-way merge
  conflicts with touching the same file.
...
| Performance: enforcer call <1µs (cache hit)  | ✅ structurally (RwLock + HashMap); benchmark sweep deferred |
| `docs/plugins/security.md` written  | ✅ |
| Definition of Done passes  | ✅ (fmt, clippy --all-targets -D warnings, rustdoc -D warnings, all tests dev+release; 1194 vs the 1105 — 89 net additions) |
